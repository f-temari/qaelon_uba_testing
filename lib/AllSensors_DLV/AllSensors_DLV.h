...
private:
  ...
  float convertPressurefrompsi(float psi) {
    ...
    switch(pressure_unit) {
      ...
      case MBAR:
        return psi * 68.9475729318 / 100 / 0.75006156130264; // mbar value = psi value x 68.9475729318
      case mmHg:
        return psi * 0.689475729318; //mmHg value = psi value x 51.7149
      case PSI:
      default:
        return psi;
    }
  ...
  // Convert the input in mbar to the configured pressure output unit.
  float convertPressurefrommbar(float mbar) {
    // Serial.print("mbar:"); Serial.println(mbar);
    switch(pressure_unit) {
      case PASCAL:
        return mbar / 100.0; //1 millibar [mbar] = 100 pascal [Pa]
      case IN_H2O:
        return mbar * 2.49082; // 1 inch water (4°C) = 2.49082 millibar
      case PSI:
        return mbar * 68.947572932; //1 psi = 68.947572932 millibar
      case mmHg:
        return mbar / 1.33322; //1 millimeter mercury (0°C) = 1.33322 millibar
      case MBAR:
      default:
        return mbar;
    }
...
public:
  AllSensors_DLV_005G(TwoWire *bus) : AllSensors_DLV(bus, AllSensors_DLV::SensorType::GAUGE, 5.0) {}
};

class AllSensors_DLV_015G : public AllSensors_DLV {
public:
  AllSensors_DLV_015G(TwoWire *bus) : AllSensors_DLV(bus, AllSensors_DLV::SensorType::GAUGE, 15.0) {}
};

class AllSensors_DLV_030G : public AllSensors_DLV {
public:
  AllSensors_DLV_030G(TwoWire *bus) : AllSensors_DLV(bus, AllSensors_DLV::SensorType::GAUGE, 30.0) {}
};

class AllSensors_DLV_060G : public AllSensors_DLV {
public:
  AllSensors_DLV_060G(TwoWire *bus) : AllSensors_DLV(bus, AllSensors_DLV::SensorType::GAUGE, 60.0) {}
};
